...
- La elaboración del transmisor evidenció la importancia de separar las decisiones de control de la actualización de los registros y de evitar que una misma señal sea asignada desde distintos bloques. La lógica combinacional determina `done`, calcula el siguiente valor de `busy` mediante `next_busy` y establece cuándo avanzar al siguiente bit. El bloque secuencial registra los valores calculados de `busy`, el estado, los contadores y el registro de desplazamiento en cada flanco positivo, además de atender el reset asíncrono. Esta separación permitió corregir el control de `busy` y coordinar el avance y la finalización de la transmisión.
