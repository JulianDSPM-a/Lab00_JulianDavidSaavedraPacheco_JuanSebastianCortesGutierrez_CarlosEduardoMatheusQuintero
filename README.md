...
- Los diseños documentados muestran cómo las FSM y la representación ASM permiten relacionar decisiones de control con operaciones concretas: en el acumulador, `LOAD` borra los registros y `ADD` realiza las sumas; en el transmisor, `S_LOAD` carga el byte y `S_BIT_HOLD` coordina su envío. Las tablas de transición y de operaciones permiten seguir esa relación entre el estado y el datapath en cada ejercicio.
- En el semáforo, separar los dos intervalos amarillos en `S1` y `S3` permitió conservar su distinto destino, rojo o verde, aunque ambos activen la misma salida. La simulación de la secuencia completa verifica las duraciones de 5, 2, 4 y 2 ciclos descritas en el diseño. Así, la elección de estados distingue tanto las salidas como la etapa que debe seguirles.
- Los errores de finalización del acumulador mostraron que las condiciones deben anticipar la operación que se registra en el flanco de salida de `ADD`. Las pruebas 1 y 5 respaldan las correcciones explicadas en el informe: se obtienen tres sumas de 5 en el modo `01` y un resultado de 20 en el modo por umbral, sin la suma adicional que producían las comparaciones iniciales.
- Las pruebas de cancelación y sumando cero verifican las condiciones de retorno a `LOAD`. En la prueba 6, la cancelación impide activar `done` aunque se alcance el criterio de terminación; en las pruebas 10 a 14, el sumando cero provoca espera o reinicio y el cálculo continúa al recibir un valor distinto de cero. Estos resultados corresponden a las condiciones mutuamente excluyentes del diagrama actualizado.
- Las pruebas 4 y 7 confirman la equivalencia de los modos `00` y `11`: con sumando 6, ambos terminan en 24 después de cuatro sumas. Las pruebas 8 y 9 muestran que los modos de tres y cuatro sumas admiten el sumando máximo de 15, con resultados de 45 y 60 que caben en los seis bits de `acc`. La prueba 15 comprueba que el contador de cinco bits representa las veinte sumas necesarias para alcanzar el umbral con sumando 1.
- En el transmisor, la separación descrita entre el cálculo combinacional de `done` y `next_busy` y la actualización secuencial de `busy` se refleja en la captura de finalización: a los 365 ns, `busy` baja al mismo tiempo que `done` sube, y este último vuelve a cero a los 375 ns. El detalle de `A5` también muestra cuatro ciclos por bit y la reconstrucción del byte transmitido. Estas evidencias respaldan la coordinación de temporización y señales de control en el diseño implementado.
- Las pruebas de reset muestran una recuperación definida en los casos ensayados: el semáforo vuelve a verde desde cada uno de sus cuatro estados, mientras que el transmisor aborta el envío de `96` sin activar `done` y luego transmite `3C` correctamente. Las comprobaciones automáticas y las capturas documentan tanto la interrupción como la continuación de la operación, en lugar de limitar la verificación al funcionamiento sin reset.
